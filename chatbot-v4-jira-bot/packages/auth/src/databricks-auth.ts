...
export async function getAuthSession({
  ...
  try {
    ...
    // Local development shortcut: if DATABRICKS_TOKEN is set directly (PAT),
    // skip the SCIM/CLI lookup and use JIRA_EMAIL as the user identity.
    // This avoids the need for Databricks CLI to be installed and configured locally.
    if (process.env.DATABRICKS_TOKEN && !process.env.DATABRICKS_CLIENT_ID) {
      const email = process.env.JIRA_EMAIL ?? 'user@innoceanusa.com';
      const username = email.split('@')[0];
      console.log('[getAuthSession] Using DATABRICKS_TOKEN PAT with JIRA_EMAIL for local dev');
      return {
        user: {
          id: email,
          email,
